...
from flags import getFlags_RNN_Classifier

# Import predefined RNN cells
...
class Model(object):
    ...
    def classifier(self, x, training=True, reuse=None, name=None):
        ...
        # Reshape and unstack for LSTM layers
        x = tf.unstack(tf.reshape(x, [-1, 28, 28]), 28, 1)

        # Define LSTM cells with dropout wrappers
        cells = rnn.DropoutWrapper(rnn.BasicLSTMCell(128, activation=tf.nn.leaky_relu, reuse=reuse, name='lstm_cells'),
                                   input_keep_prob=1.0, output_keep_prob=0.8)

        # Compute output list and final hidden state
        h_list, state = rnn.static_rnn(cells, x, dtype=tf.float32)

        # [None, 28] -->  [None, label_count]
        h = dense(h_list[-1], self.label_count, reuse=reuse, name='c_dense_1')
